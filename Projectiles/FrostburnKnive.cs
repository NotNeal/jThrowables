    ...
    public class FrostburnKnive : ModProjectile
    {
        ...
        public override void PostAI()
        {
		    ...
		    float speedY = projectile.velocity.Y * (float)Main.rand.Next(5) * 0.3f;
                ...
                if (Main.rand.Next(3) == 0)
                    {
                    int i = Dust.NewDust(projectile.position, projectile.width, projectile.height, 67, speedX, speedY, 80, default(Color), 1.2f);
                    Main.dust[i].noGravity = true;
                    int j = Dust.NewDust(projectile.position, projectile.width, projectile.height, 67, speedX, speedY, 65, default(Color), 0.8f);
        ...
        public override void PostKill()
        {
            ...
            Main.PlaySound(0, (int)projectile.position.X, (int)projectile.position.Y, 1);
                ...
                if (Main.rand.Next(3) == 0)
                {
                int hk = ItemDef.byName["jThrowables:Frostburn Knive"].type;
